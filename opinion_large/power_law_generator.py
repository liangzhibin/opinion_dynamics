...
def randht(n,*varargin):
    ...
    while i<len(varargin): 
        ...
        if not argok: 
            print('(RANDHT) Ignoring invalid argument #')
            print(i+1)
        i = i+1 
...
with open('./u') as file_object_u:
    lines_u = file_object_u.readlines()
lines_u = [float(i) for i in lines_u]
u_len = len(lines_u)

with open('./l') as file_object_l:
    lines_l = file_object_l.readlines()
lines_l = [float(i) for i in lines_l]
l_len = len(lines_l)

assert(u_len == l_len)

with open('./alpha', 'wt') as fileout:
    for idx in range(u_len):
        while True:
            # power law distribution with most coordinates being high values
            r = randht(1,'powerlaw', -2)

            # uncomment this line to get power law distribution with most coordinates being low values
            # a[0] = 1.0 - a[0]

            if r[0] <= lines_u[idx] and r[0] >= lines_l[idx]:
                print(r[0], file=fileout)
                break
